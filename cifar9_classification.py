...
# remove the last class for the dataset
# train
# set of index such that the array would be ordered
order_index_set_train = np.argsort(output_train, axis=0)
output_train = np.delete(output_train, order_index_set_train[45000:], 0)
input_train  = np.delete(input_train, order_index_set_train[45000:], 0)


# test
# set of index such that the array would be ordered
order_index_set_test = np.argsort(output_test, axis=0)
output_test = np.delete(output_test, order_index_set_test[9000:], 0)
input_test  = np.delete(input_test, order_index_set_test[9000:], 0)
...
plot.plot_training_loss(history, show=False, save_file="plots/cifar9_loss.png")
plot.plot_training_accuracy(history, show=False, save_file="plots/cifar9_accuracy.png")
